...
class JSPage(object):
	...
	# Set the eof marker if there is no more input.
	def read_some(self):
		if self.eof: return
	...
	# Put a string into the output buffer. If the total length of
	# the output buffer is at least BLKSIZE, write it to the
	# output stream.
	def output(self, s):
	...
	def rewrite_part(self, s):
		...
		if scheme:
			s = "".join((scheme, hostname, ".", self.config.hostname, portstr, s[m.end():]))
		else:
			s = "".join((scheme, hostname, ".", self.config.hostname, m.group(4) or '', s[m.end():]))
	...
	def rewrite(self):
		...
		# Read the first block to make sure there is some data
		# to work with.
		self.read_some()
		...
		while True:
			...
			# Too much data without a quoted string match:
			# stop and flush the remainder.
			if len(s) >= max_page_size: break

			# Find the next unescaped quote character
			m = re_scan.match(s, p)
			if not m:
				# None found. If there is more input,
				# read another chunk of data and try
				# again.
				if self.eof: break
				self.read_some()
				continue

			# Rewrite a possible URL or hostname in the
			# part, and advance to the next position. For
			# efficiency, the quote is included in the
			# string passed to rewrite_part, but this is
			# harmless because it will always be copied to
			# the output anyway.
			self.output(self.rewrite_part(m.group()))
			self.input_pos = m.end()

		# Write whatever is left in the input buffer, and
