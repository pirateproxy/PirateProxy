...
class Page(HtmlParser):
	BLKSIZE=65536
	tags_and_attribs_list = [(['base', 'a', 'link', 'area'], ['href']), (['audio', 'embed', 'frame', 'script', 'img', 'iframe', 'input', 'source'], ['data-thumb', 'src', 'longdesc']),(['form'], ['action']), (['applet', 'object'], ['codebase', 'archive', 'code']), (['command'], ['icon']), (['video'], ['poster', 'src']), (['button'], ['formaction'])]
	...
	def rewrite(self):
		while True:
			...
			# non-parsed data. Continue reading and parsing until
			# the end of the attribute is found (or the maximum attribute
			# size is reached). In case of script or style tags,
			# until the end of the tag or the maximum size is reached
			if self.state == self.HTMLPARSER_STATE_INT_CDATA_TEXT:
				self.handle_tag()
			elif self.state == self.HTMLPARSER_STATE_INT_VALUE_TEXT or self.state == self.HTMLPARSER_STATE_INT_VALUE_Q_START or self.state == self.HTMLPARSER_STATE_INT_VALUE_DQ_START:
				self.handle_attribute()
			else:
				pass

	# Read and parse tag contents in blocks of the configured max_page_size
	# Rewrite them based on the tag type: script or style.
	def handle_tag(self):
		# Let JSPage and CSSPage handle reading this tag. The reader
		# ensures that the page is only read until the end of the tag and
		# ensures that the input buffer does not contain the script
	...
	# This function writes data to the output buffer and writes the output
	# buffer to the client if necessary
	def buffered_writer(self, s):
		self.output_buffer += s
		self.write_output(False)

	# This function is used to read a block of data from the server response
	# and parse it until the end of the response or tag close
	# This function is used for the JSPage and CSSPage classes as reader and
	# should not update the input buffer of the Page instance with data
	# within <script> and <style> tags. 
	def parsing_reader(self, BLKSIZE):

		
		# Previously we already read until the end tag, so return nothing
		if self.state == self.HTMLPARSER_STATE_INT_TEXT:
			return None

		# We are already at the end of the input buffer and there's nothing
		# more. Empty the input buffer and return.
		if self.reached_end():
			self.input_buffer = ''
			return None
		...
		# input buffer as this will not be handled by JSPage or CSSPage
		if self.state == self.HTMLPARSER_STATE_INT_TEXT:
	...
	def handle_attribute(self):
		# The max_page_size is set to 5MB by default. If an attribute is
		# larger than this.. well, too bad.
		attr = self.attr()
		self.keep_reading(self.config.max_page_size, [self.HTMLPARSER_STATE_INT_ATTR_SPACE, self.HTMLPARSER_STATE_INT_TAG_CLOSE, self.HTMLPARSER_STATE_INT_TAG_SPACE])

		pos = self.get_position()-1

		if attr == 'style':
			# CSSPage needs read and write functions. Simulate
			...
			# JSPage needs read and write functions. Simulate
			# this by using StringIO on the already read value
			f = StringIO(self.input_buffer[:pos])
			outf = StringIO()
			...
			val = re.sub(self.simple_attriburl_regex, self.rewrite_url, self.input_buffer[:pos])
	...
	# Keep reading from the input stream in blocks until one of the requested
	# states is reached or the maximum size is passed. Fill the input buffer
	# with the data and return. Do not write data to the output buffer,
	# because this data is going to be rewritten.
	# 
	# The first time in the loop, use the current input buffer and do not
	# read any data yet
	...
	def parse_until_interesting(self):
		while True:
			...
			# Nothing interesting so far, write everything to the client
			# and continue with next block.
			self.output_buffer += s
			self.input_buffer = ''
			self.write_output(False)

	# Write output buffer in BLKSIZE blocks to the client. If final is
	# set, write everything, including the last block even if it is smaller
	# than BLKSIZE
	# 
	# Remove written blocks from the output buffer
